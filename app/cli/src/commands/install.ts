...
`;
        ...
        addMlnexusEntry(packageName, manifest.version);
        ensurePostinstall();
        ...
        if (manifest.has_stream) {
            log.plain(
                "  const stream = await model.stream({ /* prompt/input */ });",
            );
            log.plain(
...
async function ensureRuntimeDeps(): Promise<void> {
    ...
    const runtimeExists = existsSync(
        join(nodeModules, "@mlnexus", "runtime", "package.json"),
    );
    ...
    const deps: string[] = [];
    if (!runtimeExists) deps.push("@mlnexus/runtime");
