...
program
    ...
    .alias("rm")
    .description("Remove an mlnexus package from your project")
    .action(async (name: string) => {
...
program
    .command("restore")
    .description("Restore all mlnexus packages from package.json (runs in postinstall)")
    .action(async () => {
...
program
    .command("list")
    .alias("ls")
    .description("List all mlnexus packages in this project")
    .action(async () => {
        await listCommand();
    });
...
const cache = program
    .command("cache")
    .description("Manage the global model cache (~/.mlnexus/cache/)");
