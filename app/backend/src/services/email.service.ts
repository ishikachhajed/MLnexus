...
export async function sendOtpEmail(to: string, otp: string) {
    const res = await fetch("https://api.brevo.com/v3/smtp/email", {
        ...
        body: JSON.stringify({
            sender: { email: env.SENDER_EMAIL, name: "MLnexus Verification" },
            to: [{ email: to }],
            subject: "Your MLnexus verification code",
...
export async function sendPasswordResetOtpEmail(
    ...
    const res = await fetch("https://api.brevo.com/v3/smtp/email", {
        ...
        body: JSON.stringify({
            sender: { email: env.SENDER_EMAIL, name: "MLnexus Support" },
            to: [{ email: to }],
            subject: "Your MLnexus password reset code",
